...
export function resetRun(
  ...
  if (mode === 'explore') {
    ...
    world.seed = (Math.random() * 0xffffffff) >>> 0
...
export function startRun(world: DriftWorld) {
  if (world.phase === 'idle' || world.phase === 'failed' || world.phase === 'cleared') {
    resetRun(world)
  }
}
...
export function stepWorld(world: DriftWorld, dt: number, input: DriftInput) {
  ...
  for (let step = 0; step < steps; step++) {
    ...
    world.score = calculateInfiniteScore(world.distance, world.elapsed)
...
export function playerWorldPos(world: DriftWorld): {
  ...
  const out = FACE_OUTWARD[world.face]!
  const lat = FACE_LATERAL[world.face]!
  const half = TUNNEL_HALF
  const lateralDist = world.lateral * (half * 2)
  // Surface sits at outward * half; up is -outward
  const x = out[0]! * (half - world.height) + lat[0]! * lateralDist
  const y = out[1]! * (half - world.height) + lat[1]! * lateralDist
  const z = world.z
